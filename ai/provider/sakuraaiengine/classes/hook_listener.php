...
class hook_listener {
    ...
    public static function set_form_definition_for_aiprovider_sakuraaiengine(after_ai_provider_form_hook $hook): void {
        ...
        $mform->addElement(
            'passwordunmask',
            'account_token',
            get_string('account_token', 'aiprovider_sakuraaiengine'),
        );
        $mform->setType('account_token', PARAM_TEXT);
        $mform->addHelpButton('account_token', 'account_token', 'aiprovider_sakuraaiengine');
    }
    ...
    public static function set_model_form_definition_for_aiprovider_sakuraaiengine(
        ...
        if (isset($mform->_elementIndex['modeltemplate'])) {
            ...
            if ($model === 'custom') {
                $mform->addElement('header', 'modelsettingsheader', get_string('settings', 'aiprovider_sakuraaiengine'));
                $mform->addElement('html', get_string('settings_help', 'aiprovider_sakuraaiengine'));
                $mform->addElement(
                    'textarea',
                    'modelextraparams',
                    get_string('extraparams', 'aiprovider_sakuraaiengine'),
                    ['rows' => 5, 'cols' => 20],
                );
                ...
                $mform->addElement(
                    ...
                    null,
                    get_string('extraparams_help', 'aiprovider_sakuraaiengine')
                );
            } else {
